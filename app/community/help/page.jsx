                    ...
                    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-sm">
                        ...
                        {scope === "course" && (
                            <label className="block mb-2">
                                ...
                                <input value={course} onChange={(e) => setCourse(e.target.value)} placeholder="e.g., CSE101" className="mt-1 block w-full rounded-md border-gray-200" />
                            </label>
                        )}

                        <label className="block mb-2">
                            <span className="text-sm font-medium">Problem Title</span>
                            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Short summary" className="mt-1 block w-full rounded-md border-gray-200" />
                        </label>

                        <label className="block mb-4">
                            <span className="text-sm font-medium">Details</span>
                            <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={5} placeholder="Describe the problem and what you've tried" className="mt-1 block w-full rounded-md border-gray-200" />
                        </label>

                        <div className="flex gap-2">
                            <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded">Post</button>
                            <button type="button" onClick={() => { setTitle(""); setBody("") }} className="px-4 py-2 rounded border">Clear</button>
                            <button type="button" onClick={handleClear} className="ml-auto text-sm text-red-600">Clear All for {scope === "course" ? "Course" : "Research/Career"}</button>
                        </div>
                    </form>
                </div>
